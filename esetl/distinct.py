...
keys_to_include_in_hash = ["mac", "servicecode", "stime","etime","source"]
...
def scroll_over_all_docs():
    for hit in helpers.scan(es, index='mactrace202006'):
...
def loop_over_hashes_and_remove_duplicates():
    ...
    for hashval, array_of_ids in dict_of_duplicate_docs.items():
        if len(array_of_ids) > 1:
            ...
            matching_docs = es.mget(index="mactrace202006", doc_type="mactrace", body={"ids": array_of_ids})
            for doc in matching_docs['docs']:
                ...
                es.delete(index="mactrace202006",doc_type="mactrace",id=doc['_id'])
                print("doc=%s\n" % doc)
