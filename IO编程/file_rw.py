...
"""
path = 'D:\\Users\\futanghang004\\testhaha.log'
f = open(path, 'r')

"""
标示符'r'表示读，这样，我们就成功地打开了一个文件。

如果文件不存在，open()函数就会抛出一个IOError的错误，并且给出错误码和详细的信息告诉你文件
不存在
"""
"""
...
1.可以反复调用read(size)方法，每次最多读取size个字节的内容。
...
"""
flumePath = 'D:\\Users\\Public\\Pictures\\Sample Pictures\\Koala.jpg'
f = open(flumePath, 'rb')
print(f.read())
...
"""
遇到有些编码不规范的文件，你可能会遇到UnicodeDecodeError，因为在文本文件中
可能夹杂了一些非法编码的字符。遇到这种情况，open()函数还接收一个errors参数，表示如果遇到编码错误后如何处理。最简单的方式是直接忽略：
"""
...
"""
你可以反复调用write()来写入文件，但是务必要调用f.close()来关闭文件。当我们
